...
class Sequence(Dataset):
    def __init__(self, seq_path: Path, representation_type: RepresentationType, mode: str = 'test', delta_t_ms: int = 100,
                 num_bins: int = 4, transforms=[], name_idx=0, visualize=False, load_gt=False, sequence_length=2): 
        assert num_bins >= 1
        ...
        self.sequence_length = sequence_length  # 追加: sequence_lengthを保存
    ...
    def get_data(self, index) -> Dict[str, any]:
        ...
        ts_start: int = ts_end - self.delta_t_us * self.sequence_length

        # ts_startが負の値にならないように修正
        if ts_start < 0:
            ts_start = 0
        ...
        for i in range(self.sequence_length):
            ts_start_i = ts_start + i * self.delta_t_us
            ts_end_i = ts_start + (i + 1) * self.delta_t_us
            event_data = self.event_slicer.get_events(ts_start_i, ts_end_i)
            ...
            event_representations.append(event_representation)

        # 修正: 連続フレームをスタックして入力データを作成
        if len(event_representations) > 0:
            output['event_volume'] = torch.stack(event_representations, dim=1).view(-1, self.sequence_length * self.num_bins, self.height, self.width)
        else:
            output['event_volume'] = torch.zeros(self.sequence_length * self.num_bins, self.height, self.width)
...
class DatasetProvider:
    def __init__(self, dataset_path: Path, representation_type: RepresentationType, delta_t_ms: int = 100, num_bins=4,
                sequence_length=2, config=None, visualize=False):
        ...
        for child in test_path.iterdir():
            ...
            test_sequences.append(Sequence(child, representation_type, 'test', delta_t_ms, num_bins,
                                               ...
                                               visualize=visualize,
                                               sequence_length=self.sequence_length))  # 修正: sequence_lengthを渡す
